  ...
  const getData = async () => {
    ...
    let tempRows = respData.boletas.map((r) => {
      return [
        ...
        itemListWeb(r.items),
        `$ ${insertarPuntos(r.totals?.MntTotal)}`,
        r.client?.RUTRecep,
        r.url,
        r.uid,
      ];
        ...
        return (
          <>
            <SoftButton
              variant="text"
              color="dark"
              onClick={(e) => window.open(tableMeta.rowData[5])}
            >
  ...
  if (showCard == "web") {
    card = (
      <Card>
        <ListHeader
          ...
          label="Listado Ventas Web"
          buttonText="Agregar +"
          mode="downloadWeb"
          filter={filterWeb}
        />
        <SoftBox>
          <MUIDataTable data={dataRow} columns={columns} options={muiOptions} />
        </SoftBox>
      </Card>
    );
    ...
    card = (
      <Card>
        <ListHeader
          url="/provider/create"
          label="Listado Ventas POS"
          buttonText="Agregar +"
          mode="downloadPos"
          filter={filterPos}
        />
        <SoftBox>
          <MUIDataTable data={dataRowF} columns={columnsF} options={muiOptions} />
        </SoftBox>
      </Card>
    );
  }
          ...
          <table style={tabla}>
            <tbody>
              <tr>
                ...
                <td>
                  <strong>Farmacias Oxfar</strong>
                </td>
              </tr>
              <tr>
                <td style={td}>Direccion:</td>
                <td>
                  <strong>Antonio Bellet 147, Providencia</strong>
                </td>
              </tr>
              <tr>
                <td style={td}>Fono:</td>
                <td>
                  <strong>+56 2 2437 0237</strong>
                </td>
              </tr>
              <tr>
                <td style={td}>Fecha</td>
                <td>
                  <strong>{dateFormat(venta.createdAt)}</strong>
                </td>
              </tr>
              <tr>
                <td style={td}>Forma de Pago</td>
                <td>
                  <strong>{venta.payType}</strong>
                </td>
              </tr>
              <tr>
                <td>Boleta</td>
                <td>
                  <strong>{venta.uid}</strong>
                </td>
              </tr>
            </tbody>
          </table>
